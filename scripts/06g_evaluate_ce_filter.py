"""
CE Filter eval: CE 当粗筛, cosine 做最终排序.

流程 (每个 query, 每个 B):
  1. Router → top-B 桶 → cand (C 张)
  2. CE 给 C 张全打分, 取 top-N_filter (扔掉明显不相关)
  3. 对剩下 N_filter 张算 cosine, 排序取 top-K

扫多个 N_filter, 看 recall vs speedup 权衡.
"""
...
def main():
    ...
    ap.add_argument("--n-filters", type=str, default="50,100,150,200",
                    help="comma-separated N_filter 值")
    ...
    print(f"Q={Q}, pool={N_pool}, K={K}, M={M}, mode=ce_filter")
    ...
    for N_filter in n_filters:
        ...
        cc_before = {b: 0 for b in top_B_list}   # 桶内 cand 数 (filter 前)
        cc_after = {b: 0 for b in top_B_list}    # cosine 实际算的数 (filter 后)
    ...
    all_results["rerank_mode"] = "ce_filter+cosine"
    ...
    # 打印汇总
    print(f"\n{'='*80}")
    print(f"CE Filter Results — N_filter sweep")
    print(f"{'='*80}")
    for B in top_B_list:
