"""CE Cross-Attention listwise rerank, 跟 07c listwise 用同样 dataset, 换模型."""
import argparse, sys
...
class CEDatasetListwise(Dataset):
    """Listwise: (q, [target, neg_1, ..., neg_{n_cand-1}]) with label 0."""
    def __init__(self, train_q, train_target_row, train_img, test_img,
        ...
        print(f"[Dataset] Pre-compute done in {time.time()-t0:.1f}s", flush=True)
        avg = np.mean([len(p) for p in self.cand_pools])
        print(f"[Dataset] Avg cand pool: {avg:.1f}, n_neg: {self.n_neg}", flush=True)
...
def train_xattn(model, train_loader, val_loader, epochs, lr, device, verbose=True):
    ...
    for ep in range(1, epochs+1):
        ...
        log.append({"epoch": ep, "train_loss": train_loss, "val_acc": val_acc,
                    "alpha": alpha_val, "lr": sched.get_last_lr()[0]})
        if verbose:
            print(f"[CE-XAttn Ep {ep}] loss={train_loss:.4f} val_top1={val_acc:.4f} alpha={alpha_val:.3f} ({time.time()-t0:.1f}s)", flush=True)
    return log
...
def main():
    ...
    logger = setup_logger(log_file=str(out_dir / "07d_train_ce_xattn.log"))
    ...
    logger.info(f"Training X-Attn on {device}...")
    train_log = train_xattn(model, train_loader, val_loader,
